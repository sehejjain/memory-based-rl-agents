...
from sb3.model import HELM
from sb3.utils import RolloutBuffer, get_exp_decay, get_linear_burn_in_fn
...
class HELMPPO(OnPolicyAlgorithm):
    ...
    def __init__(
        self,
        policy: Union[str, Type[ActorCriticPolicy], Type[MultiInputActorCriticPolicy]],
        ...
        if lr_decay == "none":
            ...
            self.learning_rate = get_linear_fn(
                learning_rate, min_lr, end_fraction=end_fraction
            )
        elif lr_decay == "linear_burn_in":
            self.learning_rate = get_linear_burn_in_fn(
                learning_rate,
                min_lr,
            ...
            self.learning_rate = get_exp_decay(
                learning_rate, ent_decay_factor, start_fraction=start_fraction
            )
        else:
        ...
        if ent_decay == "none":
            ...
            self.ent_coef = get_linear_fn(
                ent_coef, min_ent_coef, end_fraction=end_fraction
            )
        elif ent_decay == "linear_burn_in":
            self.ent_coef = get_linear_burn_in_fn(
                ent_coef,
                min_ent_coef,
                start_fraction=start_fraction,
                end_fraction=end_fraction,
        ...
        if clip_decay == "none":
            ...
            raise NotImplementedError(
                f"Clipping factor decay {clip_decay} not implemented"
            )
        ...
        print("helm.py: input_dim", self.observation_space.shape)
        self.policy = HELM(
    ...
    def _update_entropy_coef(self) -> None:
        ...
        if self.ent_decay == "exp":
            self.ent_coef = self.ent_coef_schedule(
                self._current_progress_remaining, self.counter
            )
        else:
    ...
    def _dump_sources(self, outpath) -> None:
        zipf = zipfile.ZipFile(
            os.path.join(outpath, "LMRL.zip"), "w", zipfile.ZIP_DEFLATED
        )
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in generator:
                ...
                if len(advantages) > 1:
                    ...
                    self.logger.record(
                        "train/adv_std", advantages.std().detach().cpu().item()
                    )
                    self.logger.record(
                        "train/adv_mean", advantages.mean().detach().cpu().item()
                    )
                    if self.adv_norm:
                ...
                approx_kl_divs.append(
                    th.mean(old_log_prob - log_prob).detach().cpu().numpy()
                )
    ...
    def collect_rollouts(
        ...
        if self._last_mems is None:
            self._last_mems = [
                torch.zeros(
                    (self.policy.mem_len, self.n_envs, self.policy.model.d_embed)
                ).to(self.device)
        ...
        while n_steps < n_rollout_steps:
            ...
            for l in range(len(self._last_mems)):
                self._last_mems[l][:, self._last_episode_starts] = 0.0
        ...
        rollout_buffer.compute_returns_and_advantage(
            last_values=value, dones=self._last_episode_starts
        )
        ...
        if self.save_ckpt:
            if len(self.ep_info_buffer) < self.n_envs:
                ...
                ret = safe_mean(ret + [0.0] * (self.n_envs - len(ret)))
            else:
    ...
    def _prepare_checkpoint(self):
        ent_coef = (
            self.ent_coef_schedule(self._current_progress_remaining, self.counter)
            if self.ent_decay == "exp"
            else self.ent_coef_schedule(self._current_progress_remaining)
        )
        lr = self.lr_schedule(self._current_progress_remaining)
        clip_range = self.clip_range_schedule(self._current_progress_remaining)
        module_names = [
            d
            for d in dir(self.policy)
            if not d.startswith("_")
            and isinstance(getattr(self.policy, d), torch.nn.Module)
            and d != "model"
        ]
        checkpoint = {
            "network": {},
    ...
    def learn(
        ...
        self.save_path = os.path.join(
            os.path.join(self.tensorboard_log, f"PPO_{latest_run}")
        )
