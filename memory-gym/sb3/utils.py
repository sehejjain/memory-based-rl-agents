...
#     def __init__(
#         self, level_name: str, action_repeats: int = 4, record_episodes: bool = False
#     ):
...
class RolloutBuffer(BaseBuffer):
    ...
    def get_obs_shape(self, observation_space):
        if self.is_dict_observation:
            return {key: space.shape for key, space in observation_space.spaces.items()}
        else:
    ...
    def reset(self) -> None:
        ...
        if self.hidden_dim:
            self.hiddens = np.zeros(
                (self.buffer_size, self.n_envs, self.hidden_dim), dtype=np.float32
            )
    ...
    def add(
        ...
        self.log_probs[self.pos] = probs.sum() if probs.ndim > 0 else probs.copy()
        self.pos += 1
...
#         hidden_dim: int = 256,
#     ):
#         super(RecurrentRolloutBuffer, self).__init__(
#             buffer_size, observation_space, action_space, device, n_envs=n_envs
#         )
#         self.gae_lambda = gae_lambda
#         self.gamma = gamma
...
#         )
#         self.hiddens = np.zeros(
#             (self.buffer_size, self.n_envs, 2, self.hidden_dim), dtype=np.float32
#         )
#         self.actions = np.zeros(
#             (self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32
#         )
...
#             padded_seqs.append(
#                 np.concatenate(
#                     (seq, np.zeros((diff, *seq.shape[1:]), dtype=np.float32))
#                 )
#             )
...
#         for t in _tensor_names:
#             self.__dict__[t], self.valid_masks = self._pad_sequences(
#                 self.__dict__[t], maxlen
#             )
