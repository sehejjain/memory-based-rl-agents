...
# import memory_gym  # type: ignore
# import memory_maze  # type: ignore
import numpy as np
import torch
...
os.sys.path.append(os.path.abspath(os.path.join(os.getcwd(), os.pardir)))  # type: ignore
...
ENV_NAME = "MemoryMaze"  # "MortarMayhem-v0"


# env = gym.make("GymV21Environment-v0", env=old_gym.make(ENV_NAME))


# eval_env = Monitor(gym.make("GymV21Environment-v0", env=old_gym.make(ENV_NAME)))
...
latest_checkpoint = None if True else find_latest_checkpoint("sb3/saved/", "rl_model")  # type: ignore
...
e_callback = EpisodicRewardCallback(plot_freq=10, log_dir="tensorboard_logs/")
...
eval_callback = EvalCallback(
    eval_env,
    best_model_save_path="sb3/logs/" + ENV_NAME,
    log_path="sb3/logs/" + ENV_NAME,
    eval_freq=30000,
    deterministic=True,
    render=False,
)
